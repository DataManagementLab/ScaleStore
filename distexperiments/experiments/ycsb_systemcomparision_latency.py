...
def ycsbBenchmark(servers, dramGB, numberNodes, initialFillDegree ,pp,fp, partitioned, evict_ssd, RUNS):
    ...
    for i in range(0, numberNodes):
        cmd = f'numactl --membind=0 --cpunodebind=0 ./ycsb_latency -worker=20 -dramGB={dramGB} -nodes={numberNodes} -messageHandlerThreads=4   -ownIp={servers[i].ibIp} -pageProviderThreads={pp} -coolingPercentage=10 -freePercentage={fp} -csvFile=ycsb_latency_syscompare_scalestore_{RUNS}.csv -YCSB_run_for_seconds=30 -YCSB_tuple_count={numTuples} -{partitioned} -{evict_ssd} -tag={partitioned}{evict_ssd} -evictCoolestEpochs=0.1  -ssd_path={servers[i].ssdPath} -ssd_gib=2000 -YCSB_all_workloads -YCSB_all_zipf -YCSB_record_latency -YCSB_flush_pages -YCSB_warm_up'
