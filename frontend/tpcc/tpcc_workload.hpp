...
void loadOrders(Integer w_id, Integer d_id) {
   ...
   for (Integer o_c_id : c_ids) {
      ...
      if (FLAGS_order_wdc_index) { order_wdc.insert({w_id, d_id, o_c_id, o_id}, {}); }
...
void newOrder(Integer w_id,
   ...
   if (FLAGS_order_wdc_index) { order_wdc.insert({w_id, d_id, c_id, o_id}, {}); }
...
void newOrderRnd(Integer w_id) {
   ...
   for (Integer i = 1; i <= ol_cnt; i++) {
      ...
      if (urand(1, 100) == 1) {  // remote transaction
         ...
         remote_new_order++;
         if (supware <= (Integer)warehouse_range_node.begin || (supware > (Integer)warehouse_range_node.end)) remote_node_new_order++;
      }
...
/*
void tx(Integer w_id) {
   // int rnd = scalestore::utils::RandomGenerator::getRand(0, (int)transaction_types::MAX);
   int rnd = transaction_types::DELIVERY;
   switch (rnd) {
      case transaction_types::NEW_ORDER: {
         newOrderRnd(w_id);
         txns[transaction_types::NEW_ORDER]++;
         break;
      }
      ...
      case transaction_types::STOCK_LEVEL: {
         stockLevelRnd(w_id);
         txns[transaction_types::STOCK_LEVEL]++;
         break;
      }
      case transaction_types::ORDER_STATUS: {
         orderStatusRnd(w_id);
         txns[transaction_types::ORDER_STATUS]++;
         break;
      }
